...
## Group 2 — tests (direct + FD–FFT widen + manufactured-solution acceptance)

Three test layers, all landing together in a single commit since two of them
share the new test file:
...
- New `test/spectral_wavenumbers_test.cpp`:
  ...
  - **Manufactured-solution highest-positive-harmonic acceptance**: for an odd `Ny`, `psi(y) = sin(k_max · y)` with `k_max = 2π * (Ny − 1)/2 / L`; `spectral::laplacian(psi)` must equal `−k_max^2 · psi` to round-off. Repeat for the y- and z-axis on odd grids. Without the fix the test fails by `O(|k_max|^2)` magnitude (the mode flipped to a different frequency); with the fix it passes to round-off.
...
**Commit message:** `test(spectral): cover odd-N wavenumbers + highest-harmonic acceptance + N=17 in FD-FFT sweep`

## Group 3 — version bump + CHANGELOG + STATUS
