...
- **Full LaTeX docs toolchain (Phase 10 onward).** The User Guide is built via `latexmk -xelatex` from `docs/user-guide/main.tex` (`memoir` class). The Developer Note is built via `latexmk -xelatex` from `docs/developer-note/main.tex` (`book` class) and `\input{}`s the Doxygen-generated API reference content (extracted from `refman.tex` by `docs/extract-api-reference.sh`). Doxygen runs over `include/gridcalc/` with `WARN_IF_UNDOCUMENTED=YES` and Graphviz `dot` enabled. The CI `docs-build` job installs `texlive-full` + `latexmk` + `doxygen` + `graphviz` and produces three artifacts (User Guide PDF, Developer Note PDF, Doxygen tarball) in the `gridcalc-docs-pdfs` bundle on every PR. The CI `docs-lint` job pairs Doxygen's `WARN_IF_UNDOCUMENTED` with `scripts/check-since.py` regex over public headers and fails the build on any missing `\since` tag. Local entry point: `scripts/build-docs.sh`. Engine: **xelatex**, switched from `pdflatex` during Phase 10's CI debug because TeX Live 2023 (Ubuntu CI) tripped on UTF-8 inside `\lstinline!...!` even with `inputenc utf8` + `\DeclareUnicodeCharacter` mappings; xelatex handles Unicode natively via `fontspec`. The preamble in `docs/common/preamble.tex` is engine-aware (`\usepackage{iftex}` branch) so `pdflatex` still works in environments that have it. Pandoc was used once during Phase 10 for the markdown→LaTeX migration; it is **not** a runtime requirement of the project now. The pre-Phase-10 lightweight render (`scripts/render-docs.sh` + the CI `render-docs` job) is retired.
- **Doxygen `STRIP_FROM_PATH` policy.** The `Doxyfile.in` sets `STRIP_FROM_PATH = "@CMAKE_SOURCE_DIR@/.."` and `STRIP_FROM_INC_PATH = "@CMAKE_SOURCE_DIR@/include"`. Generated documentation displays file references as `grid-calculus/include/gridcalc/...` rather than the absolute developer-machine prefix (e.g., `/Users/<user>/Projects/.../`) or the CI-runner prefix (`/home/runner/work/...`). The repo-root directory name appears as the leading visible component of every path; `#include` directives in the rendered API reference are written with the form `<gridcalc/...>`. This keeps the generated PDFs reproducible across machines (no leaked machine-specific paths) and matches the public-include convention C++ consumers see in the source.
