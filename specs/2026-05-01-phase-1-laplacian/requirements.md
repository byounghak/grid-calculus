...
- **Acceptance-test inputs.**
  - **Eigenvalue test:** $L = 2\pi$, $N_x = N_y = N_z = 32$, $\mathbf{k} = (1, 2, 1)$. Eigenvalue $-(k_x^2 + k_y^2 + k_z^2) = -6$. Tolerance: relative max-norm error $< 5\times 10^{-3}$ (2nd-order error at $h = 2\pi/32 \approx 0.196$ on $\sin$ inputs; this leaves headroom).
